...
An environment defines the cloud sandbox runs execute in: base image, image build steps, cached setup commands, resource limits, and env vars.
...
A profile is a reusable agent definition: harness, environment, and durable instructions.
...
metadata:
  name: reviewer
spec:
  harness: claude-code            # the agent harness (currently: claude-code)
  environment: agent-runtime      # environment name
  instructions: |                 # ≤100k chars; the agent's standing orders
