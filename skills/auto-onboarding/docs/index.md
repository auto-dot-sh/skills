...
2. **The event half.** Provider connections (GitHub, Slack, Linear, Telegram), custom webhooks, and cron heartbeats emit events. Session triggers match events and either **spawn** a new run or **deliver** the event to a live one. Runs are durable, observable executions of an agent.
...
| `session` | `.auto/sessions/` | The runnable workflow: binds a profile to an initial prompt, repo mounts, a tool set, an identity, and triggers. |

A fifth directory, `.auto/assets/`, holds avatar images referenced by session identities.
...
## Sessions are the center of gravity

A session answers five questions:
...
Inside a run, the agent has its mounted repo, ordinary shell access in its sandbox, and the tools the session granted:
...
- **`auto.*`** — coordinate with the platform itself: spawn sibling sessions, list and read other runs, subscribe to chat threads, record artifact ownership.
- **`checks.*`** — report a GitHub check run (begin/success/failure) when a PR trigger declared one.
- **GitHub MCP tools** — a curated, capability-scoped GitHub API surface brokered from the session's mounts.
- **Remote MCP tools** — whatever `tool` resources the session references (Notion, Datadog, ...).
