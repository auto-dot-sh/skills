...
- **Trigger**: `github.pull_request.opened|reopened|synchronize` on the user's repo, `routing: spawn` — every PR head gets a fresh review run. PR issue comments, PR reviews, and review comments with `$.github.auto.authored: false` route back through the `bind` route on the `github.pull_request` target.
