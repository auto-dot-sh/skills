...
- Add a CRM as a remote MCP tool (`kind: mcp_remote` pointing at the CRM's MCP server) and extend the profile to log dossiers and dispositions there — see `docs/tools-and-connections.md`.
- Encode the team's actual ICP (ideal customer profile) and messaging guidelines in the profile; that's what makes drafts sound like the company instead of a template.
