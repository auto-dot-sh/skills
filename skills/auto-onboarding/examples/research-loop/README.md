...
- **Experiments are isolated and honest**: each experimenter gets its own sandbox and checkout, implements exactly one variant, sessions the measurement protocol from the brief (same warmup, same iterations, baseline + variant), and reports numbers — explicitly including null and negative results. Experimenters never open PRs during the loop.
