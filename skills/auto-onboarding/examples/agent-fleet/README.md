...
- **Event-driven waiting, no polling**: after opening its PR, a staff engineer records ownership (`auto.artifacts.record`) and _ends its turn_. Check failures, aggregate CI success, review comments, and merge conflicts on that PR wake it via `ownedArtifact` delivery. The PR-conversation trigger deliberately has no `$.auto.authored: false` filter — the staff engineer must receive the pr-review agent's comments, which are also auto-authored — so its own PR comments cost one no-op wake-up each; don't "fix" the filter or it goes deaf to its reviewer. The chief's heartbeat sweeps the fleet for stalls and respawns dead sessions.
